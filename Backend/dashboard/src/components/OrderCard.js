...
function OrderCard(props)
{
    ...
    {
        axios.post(`http://localhost:8080/orders/update/${items['_id']}`, {'status': status})
    ...
    return(
        <div className='container p-1 col-xxl-6 col-xl-6 col-md-6 col-sm-12 flex-nowrap m-0'>
            <div className='shadow-lg p-3 rounded rounded-4 h-100'>
               ...
               {items['items'].map((item, key) => <OrderItem item={item} price={items['prices'][key]} sc={true}/>)}
               <div className='p-2'>
               ...
               <div>
                    <Accordion defaultActiveKey="1" className='d-flex flex-column gap-3 mb-2'>
               ...
               <div>
                    <h5>Change Status:</h5>
                    <Form.Select aria-label="Default select example" name='statusSelector' value={status} onChange={(e) => setStatus(e.target.value)}>
                        <option>Open this select menu</option>
                        <option name="statusSelector" value="Pending">Pending</option>
                        <option name="statusSelector" value="Confirmed">Confirmed</option>
                        <option name="statusSelector" value="Out for Delivery">Out for Delivery</option>
                        <option name="statusSelector" value="Delivered">Delivered</option>
                    </Form.Select>
               </div>
               <button onClick={handleChange} className='btn btn-success mt-2'>Update</button>
            </div>
        </div>
    );
}
