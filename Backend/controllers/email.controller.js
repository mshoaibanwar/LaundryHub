  ...
  User.findOne({ email })
    .then(async user => {
      ...
      // clicked on the confirmation link. Send another confirmation email.
         ...
         res.status(400).send("Email Already Registered but not confirmed. Login with your details, and confirmation email will be sent to you after login!")
      }
  ...
  User.findOne({ email })
    .then(user => {
      ...
      // user and let them know their email address has been confirmed.
        ...
        res.send({'msg': 'Email with link to reset password is sent!', 'found': 'true' });
      }
